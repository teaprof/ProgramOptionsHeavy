...
class CompleterOld
{
  ...
  private:
    ...
    MatchResults getCompletionVariants(const std::string_view &word,
    ...
    std::tuple<std::string_view, std::string_view, std::vector<std::string_view>> byRoles(
    ...
    std::vector<std::string> getOptionNames(const boost::program_options::option_description &opt)
    {
        ...
        std::string short_name =
            opt.canonical_display_name(boost::program_options::command_line_style::allow_dash_for_short);
        if (!short_name.empty())
        {
