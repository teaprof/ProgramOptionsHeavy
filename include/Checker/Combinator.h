...
class Combinator2 : public AbstractOptionVisitor {
   public:
    ...
    virtual void visit(std::shared_ptr<AbstractPositionalOption> opt) { visit(std::dynamic_pointer_cast<AbstractOption>(opt)); }
    virtual void visit(std::shared_ptr<NamedOption> opt) { visit(std::dynamic_pointer_cast<AbstractOption>(opt)); }
    virtual void visit(std::shared_ptr<LiteralString> opt) { visit(std::dynamic_pointer_cast<AbstractOption>(opt)); }
...
class Combinator {
   public:
    ...
    struct Iterator {
        size_t idx;
        Combinator& parent;
        std::vector<std::shared_ptr<AbstractOption>> operator*() { return parent.branches[idx]; }
        void operator++() { idx++; }
        bool operator!=(const Iterator& other) const { return idx != other.idx; }
    };
    Iterator begin() { return Iterator{0, *this}; }
    Iterator end() { return Iterator{branches.size(), *this}; }
};
