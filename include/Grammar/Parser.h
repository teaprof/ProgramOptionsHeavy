...
class ArgLexer
{
  ...
  public:
    ...
    static constexpr char long_option_eq_value_pattern[] =
        "^--([A-Za-z0-9_][A-Za-z0-9_\\-]*)=(.*)$";                                     // like --long_name=value
    ...
    static Result lex(const std::string &arg, bool match_only_positional)
    {
        ...
        else if (std::regex_match(arg, match, double_dash_ex))
        { // TODO: does we need to check match_only_positional flag here?
...
class ArgGrammarParser
{
  ...
  public:
    bool match_only_positional{false}; // when double dash "--" is encountered among the argument list, all following
                                       // options are treated as positional
    enum TokenTypes
    {
    ...
    template <class... Type> ArgGrammarParser(Type... arguments) : args_{arguments...}
    {
    }
    ...
    bool eof()
    {
        return idx_ == args_.size() && results_.empty();
    }
    ...
    Result getNextOption()
    {
        ...
        if (results_.empty())
        {
            ...
            case ArgLexer::LONG_OPTION_EQ_VALUE:
                results_.push_back(Result{TokenTypes::LONG_OPTION_EQ_VALUE, lex_result.long_option_name, "",
                                          lex_result.value, cur_idx});
                break;
    ...
    void ungetOption()
    {
        results_.push_front(current_result);
    }

    /// the raw option name is used in error messages to address the option by the name that was actually passed (short
    /// or long)
    ...
    { /// todo: opt is only used to print an error message
        switch (current_result.token_type)
        {
        case TokenTypes::SHORT_OPTION_WITHOUT_VALUE: {
