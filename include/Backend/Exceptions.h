...
{
  public:
    BaseOptionError(std::shared_ptr<AbstractOption> opt)
        : std::runtime_error(BaseOptionError::description(opt)), opt_{opt}
    {
    }
...
{
  public:
    ExpectedOption() : BaseOptionError(nullptr)
    {
    } //"Expected option") {}
};

class UnknownNamedOption : public BaseOptionError
{ /// TODO: make abstract base class to avoid direct inheritance from std::runtime_error
  public:
    UnknownNamedOption(std::string str) : BaseOptionError(nullptr)
    {
...
class RequiredOptionIsNotSet : public BaseOptionError
{
  public:
    ...
    {
    } // std::runtime_error("required option is not set") {}
};
...
{
  public:
    TooFewPositionalOptions() : BaseOptionError(nullptr)
    {
    } //::runtime_error("too few positional options are specified") {}
};

class TooManyPositionalOptions : public BaseOptionError
{
  public:
    TooManyPositionalOptions(const std::string &str) : BaseOptionError(nullptr)
    {
    } //::runtime_error("too many positional options are specified: {str}") {}
};

class UnexpectedValueForPositionalOption : public BaseOptionError
{
  public:
    UnexpectedValueForPositionalOption(const std::string &str) : BaseOptionError(nullptr)
    {
    } //::runtime_error("too many positional options are specified: {str}") {}
};
...
{
  public:
    TooFewValuesForOption() : BaseOptionError(nullptr)
    {
    } // too few values for options, expected at least N or exact N
};
