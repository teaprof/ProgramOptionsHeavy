...
class OptionsFacade
{
  public:
    template <class T>
    std::shared_ptr<NamedOptionWithValue<T>> makeOption(const std::string &names, std::reference_wrapper<T> storage,
    ...
    template <class T>
    std::shared_ptr<PositionalOptionWithValue<T>> makeOption(std::reference_wrapper<T> storage,
    ...
    template <class T>
    std::shared_ptr<PositionalOptionWithValue<T>> makePositionalOption(const std::string &hidden_name,
                                                                       size_t max_occurrence,
                                                                       std::reference_wrapper<T> external_storage,
                                                                       const std::string &help_message)
    {
        auto res = std::make_shared<PositionalOptionWithValue<T>>();
