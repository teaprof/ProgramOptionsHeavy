...
class SingleOptionMatcher : public AbstractOptionVisitor
{
  ...
  public:
    ...
    void visit(std::shared_ptr<NamedOption> opt) override
    {
        ...
        case ArgGrammarParser::TokenTypes::LONG_OPTION:
        case ArgGrammarParser::TokenTypes::LONG_OPTION_EQ_VALUE:
            match = opt->longName().has_value() &&
                    opt->longName().value() == grammar_parser_.current_result.long_option_name;
            break;
        case ArgGrammarParser::TokenTypes::SHORT_OPTION:
        case ArgGrammarParser::TokenTypes::SHORT_OPTION_WITHOUT_VALUE:
        case ArgGrammarParser::TokenTypes::SHORT_OPTION_EQ_VALUE:
            match = opt->shortName().has_value() &&
                    opt->shortName().value() == grammar_parser_.current_result.short_option_name;
            break;
        case ArgGrammarParser::TokenTypes::DOUBLE_DASH:
        case ArgGrammarParser::TokenTypes::VALUE:
            break;
...
class BaseMatcher
{
  ...
  public:
    ...
    bool eatNextValueIfCan(ArgGrammarParser &args, SingleOptionMatcher &matcher,
                           std::shared_ptr<AbstractOptionWithValue> opt)
    {
    ...
    std::shared_ptr<AbstractOption> eatNextToken(ArgGrammarParser &args, SingleOptionMatcher &matcher)
    {
        ...
        for (auto it : remaining_options_)
        {
            ...
            if (matcher.match)
            {
                ...
                {
                    joinOptionsTo(matcher.unlocks, remaining_options_); // TODO: what should we do if this option was
                                                                        // unpacked (multiple occurrence)
    ...
    { // TODO:  rename (Parser is another class)
        clear();
        SingleOptionMatcher matcher(args);
        while (!args.eof())
        {
  ...
  protected:
    ...
    void addValueToNewOccurrence(std::shared_ptr<AbstractOption> opt, const SingleOptionMatcher &matcher,
                                 std::vector<std::shared_ptr<AbstractOption>> &unlocked_by_value)
    {
    ...
    void addValueToCurrentOccurrence(std::shared_ptr<AbstractOptionWithValue> opt, const std::string &value,
                                     std::vector<std::shared_ptr<AbstractOption>> &unlocked_by_value)
    {
    ...
    { // TODO: rename to something like checkIfValueListIsCompleted
